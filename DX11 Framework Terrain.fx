...
Texture2D txSand : register(t0);
Texture2D txGrass : register(t1);
Texture2D txRock : register(t2);
Texture2D txSnow : register(t3);
Texture2D txBlend : register(t4);
...
float4 PS(PS_INPUT input) : SV_Target
{
    ...
    float4 textureBlend = txBlend.Sample(samLinear, input.Tex);

    if (input.PosW.y < 2.0f)
    {
        textureColour = txSand.Sample(samLinear, input.Tex);
    }    
    else if (input.PosW.y < 4.0f && input.PosW.y >= 2.0f)
    {
        textureColour = txGrass.Sample(samLinear, input.Tex);
    }        
    else if (input.PosW.y < 6.0f && input.PosW.y >= 4.0f)
    {
        textureColour = txRock.Sample(samLinear, input.Tex);
    }
    else if (input.PosW.y >= 6.0f)
    {
